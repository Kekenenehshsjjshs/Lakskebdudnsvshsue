...
ffi.cdef[[
    ...
    typedef struct { unsigned char r,g,b,a; } RwColor;
    ...
    void  _ZN9CRenderer22RenderFadingInEntitiesEv();
    void  _Z21DeActivateDirectionalv(void);
    void  _Z14SetFullAmbientv(void);
]]

local SW, SH = getScreenResolution()
local ws = new.bool(false)

-- config (pakai jsoncfg bawaan, nama file: flatped.json)
local CN = 'chams'
...
local b1 = ffi.new("uint32_t[1]")
local b2 = ffi.new("RwColor")
local p2 = false
local q1 = {}
local r2 = {}
local en = true
...
local function rg(st)
    gt._Z16RwRenderStateGet13RwRenderStatePv(st, b1)
    return b1[0]
end

local function rt(st, v)
    gt._Z16RwRenderStateSet13RwRenderStatePv(st, cs("void*", v))
end
...
local cm = ffi.cast("void*(*)(void*, void*)", function(mp, dp)
    if mp == nil then return mp end
    local m = cs("RpMaterial*", mp)
    local c = cs("RwColor*", dp)
...
local ca = ffi.cast("void*(*)(void*, void*)", function(ap, dp)
    if ap == nil then return ap end
    local a = cs("RpAtomic*", ap)
    local g = a.geometry
    if g ~= nil then
        g.flags = bit.bor(g.flags, GM)
        gt._Z25RpGeometryForAllMaterialsP10RpGeometryPFP10RpMaterialS2_PvES3_(g, cm, dp)
    end
    return ap
end)
...
local function af(cl, r, g, b, a)
    ...
    gt._Z20RpClumpForAllAtomicsP7RpClumpPFP8RpAtomicS2_PvES3_(cl, ca, b2)
end
...
local function r3()
    ...
    p2 = true
    gt._Z21DeActivateDirectionalv()
    gt._Z14SetFullAmbientv()
    ...
    for _, pd in ipairs(qq) do
        if pd ~= nil then
            local md = (tonumber(cs("intptr_t", pd)) or 0) % 360
            local r,g,b,a = gc(md)
            rt(AM, ma(r,g,b,255))
            if pd.pRwClump ~= nil then
                af(pd.pRwClump, r, g, b, a)
            end
            gt._ZN4CPed6RenderEv(cs("CPed*", pd))
            rl()
        end
    end
...
hk = hook.new("void(*)(CPed*)", function(pd)
    ...
    local lp = gt._Z13FindPlayerPedi(0)
    local il = cs("uintptr_t", pd) == cs("uintptr_t", lp)
...
end, cs("uintptr_t", cs("void*", gt._ZN4CPed6RenderEv)))

local hf
hf = hook.new("void(*)(void)", function()
    hf()
    r3()
end, cs("uintptr_t", cs("void*", gt._ZN9CRenderer22RenderFadingInEntitiesEv)))

imgui.OnFrame(
    function() return ws[0] end,
    function()
        imgui.SetNextWindowPos(imgui.ImVec2(SW/2, SH/2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
        imgui.Begin(u8'Flat Ped Color', ws, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.AlwaysAutoResize)
